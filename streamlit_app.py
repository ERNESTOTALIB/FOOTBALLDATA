...
This version has been enhanced to support real first‑half and second‑half
metrics (goals, cards and corners) as well as referee information
enriched from the `fixtures_enriched` table. When such data is not
present, the app falls back to even splits between halves for
cards and corners.
"""
...
# Paths to local cleaned CSVs used when database access is disabled
...
# ---------------------------------------------------------------------------
# Helper functions for per‑team metrics and probability calculations
...
def calc_over_under_for_subset(
    ...
    time_frame : str, optional
        "Full match", "First Half" or "Second Half"; for cards/corners only full match is supported.
    ...
    """
    # Cards and corners are only available at full‑match level
    if category in ["Tarjetas", "Córners"] and time_frame != "Full match":
        time_frame = "Full match"

    # Determine which column to use for goals depending on time frame
    if category == "Goles":
    ...
    # For cards and corners define metric and column mapping by scope/time frame
    metric = None
    ...
    if metric is not None:
        # Determine column based on metric, scope and time frame
        if metric == "cards":
            if scope == "Total match":
                col = "cards_total" if time_frame == "Full match" else (
                    "cards_first_half" if time_frame == "First Half" else "cards_second_half"
                )
            elif scope == "Home":
                col = "cards_home" if time_frame == "Full match" else (
                    "cards_home_first_half" if time_frame == "First Half" else "cards_home_second_half"
                )
            else:  # scope == "Away"
                col = "cards_away" if time_frame == "Full match" else (
                    "cards_away_first_half" if time_frame == "First Half" else "cards_away_second_half"
                )
        else:  # corners
            if scope == "Total match":
                col = "corners_total" if time_frame == "Full match" else (
                    "corners_first_half" if time_frame == "First Half" else "corners_second_half"
                )
            elif scope == "Home":
                col = "corners_home" if time_frame == "Full match" else (
                    "corners_home_first_half" if time_frame == "First Half" else "corners_home_second_half"
                )
            else:  # scope == "Away"
                col = "corners_away" if time_frame == "Full match" else (
                    "corners_away_first_half" if time_frame == "First Half" else "corners_away_second_half"
                )

        # Total match scope: compute probability across all matches
        if scope == "Total match":
            ...
            for idx in series[fails_mask].index:
                row = df_subset.loc[idx]
                # Determine opponent relative to home and away teams
                if row.get("HomeTeam") == home_team:
                    opp = row.get("AwayTeam")
                elif row.get("AwayTeam") == home_team:
                    opp = row.get("HomeTeam")
        ...
        assigned_team = home_team if scope == "Home" else away_team
        entries = []
        for idx, row in df_subset.iterrows():
            if metric == "cards":
                if row.get("HomeTeam") == assigned_team:
                    val = row.get(
                        "cards_home" if time_frame == "Full match" else (
                            "cards_home_first_half" if time_frame == "First Half" else "cards_home_second_half"
                        ),
                        None,
                    )
                elif row.get("AwayTeam") == assigned_team:
                    val = row.get(
                        "cards_away" if time_frame == "Full match" else (
                            "cards_away_first_half" if time_frame == "First Half" else "cards_away_second_half"
                        ),
                        None,
                    )
                ...
                if row.get("HomeTeam") == assigned_team:
                    val = row.get(
                        "corners_home" if time_frame == "Full match" else (
                            "corners_home_first_half" if time_frame == "First Half" else "corners_home_second_half"
                        ),
                        None,
                    )
                    ...
                    val = row.get(
                        "corners_away" if time_frame == "Full match" else (
                            "corners_away_first_half" if time_frame == "First Half" else "corners_away_second_half"
                        ),
                        None,
                    )
            ...
            if val is None:
                continue
            entries.append((idx, val, row))
        n = len(entries)
        if n == 0:
            return {"p": None, "hits": 0, "n": 0, "fails": []}
        hits = 0
        fails_list = []
        for idx, val, row in entries:
            ok = (val > line) if side == "over" else (val < line)
            if ok:
                hits += 1
            else:
                if row.get("HomeTeam") == assigned_team:
    ...
    # Default case: invalid category
    return {"p": None, "hits": 0, "n": 0, "fails": []}


def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize/standardize column names to canonical names used by the app.

    Handles different casing, snake_case variants and common alternative names.
    """
    if df is None or df.empty:
        return df

    # Strip whitespace and build a lowercase lookup
    cleaned_cols = [str(c).strip() for c in df.columns]
    df.columns = cleaned_cols
    lower_map = {c.lower(): c for c in df.columns}

    # Define canonical mapping by lowercase keys
    canonical_by_lower = {
        "date": "Date",
        "match_date": "Date",
        "utc_date": "Date",
        ...
        "referee": "Referee",
        "ref": "Referee",
        # Half‑time goals mapping (enriched dataset)
        "hthg": "HTHG",
        "half_time_home_goals": "HTHG",
        "htag": "HTAG",
        "half_time_away_goals": "HTAG",
        # Card halves mapping (enriched dataset)
        "first_half_cards": "cards_first_half",
        "second_half_cards": "cards_second_half",
    }
...
def add_derived_cols(df: pd.DataFrame) -> pd.DataFrame:
    """
    Compute derived columns such as totals and split metrics for goals, cards and corners.

    If first‑half and second‑half metrics are provided (e.g. cards_first_half), they are used.
    Otherwise, totals are split evenly between halves.
    """
    df = df.copy()
    # Goals totals and differences
    if "FTHG" in df.columns and "FTAG" in df.columns:
        df["goals_total"] = df["FTHG"].fillna(0) + df["FTAG"].fillna(0)
        df["goal_diff"] = df["FTHG"].fillna(0) - df["FTAG"].fillna(0)
    # Ensure card columns exist
    ...
    # First‑half goals: use HTHG/HTAG if present else split evenly
    ...
    # Cards: use provided half‑time card counts if available
    if "cards_first_half" in df.columns:
        df["cards_first_half"] = pd.to_numeric(df["cards_first_half"], errors="coerce").fillna(0)
        if "cards_second_half" in df.columns:
            df["cards_second_half"] = pd.to_numeric(df["cards_second_half"], errors="coerce").fillna(0)
        else:
            df["cards_second_half"] = df["cards_total"] - df["cards_first_half"]
    else:
        df["cards_first_half"] = df["cards_total"] / 2.0
        df["cards_second_half"] = df["cards_total"] / 2.0
    # Distribute team‑specific card halves evenly (no per‑team half data available)
    ...
    # Corners: no half‑time breakdown available; split evenly
...
def compute_referee_card_stats(
    ...
    Returns overall averages and probabilities of exceeding card lines for a referee's last
    ``last_n`` matches. Also includes average cards given to the selected home and away teams.
    """
    ref_matches = df[df.get("Referee") == referee_name].sort_values("Date")
    if last_n > 0:
    ...
    series_total = ref_matches.get("cards_total", pd.Series(dtype=float)).dropna()
    for ln in lines:
        if len(series_total) > 0:
            hits = int((series_total > ln).sum())
            over_probs[ln] = hits / len(series_total)
        else:
            over_probs[ln] = None
    # Cards to specific teams
    cards_to_home_team: list[float] = []
    matches_home_team = 0
    if home_team:
        for _, row in ref_matches.iterrows():
            ...
            if pd.notna(val):
                ...
                matches_home_team += 1
    avg_cards_to_home_team = np.mean(cards_to_home_team) if cards_to_home_team else None
    cards_to_away_team: list[float] = []
    matches_away_team = 0
    if away_team:
        for _, row in ref_matches.iterrows():
            if row.get("HomeTeam") == away_team:
            ...
            if pd.notna(val):
                ...
                matches_away_team += 1
    avg_cards_to_away_team = np.mean(cards_to_away_team) if cards_to_away_team else None
    return {
        "n": n_matches,
...
def load_data(league: str) -> pd.DataFrame:
    """
    Load match data from the database or local CSVs depending on configuration.

    When USE_DB=true and DATABASE_URL is set, queries the Supabase/PostgreSQL database,
    selecting enriched fields (half‑time goals, referee and first/second half cards) via
    a left join on fixtures_enriched. Falls back to local CSVs otherwise.
    """
    ...
    if use_db and db_url:
        ...
        try:
            ...
            return pd.DataFrame()
        # Query including match_id, half‑time goals and enriched referee/card halves
        query = """
            SELECT
              ...
              m.half_time_home_goals AS hthg,
              m.half_time_away_goals AS htag,
            ...
            if cat == "Tarjetas":
                ...
                if len(refs) > 0:
                    selected_ref = st.selectbox(
                        "Referee (optional)", sorted(list(refs)), index=0, key="ref_select"
                    )
                    ...
                    if ref_stats["n"] == 0:
                        ...
                        for ln, prob in ref_stats["over_probs"].items():
                            prob_txt = "-" if prob is None else f"{prob*100:.1f}%"
                            prob_rows.append({"Line": ln, "P(> line)": prob_txt})
                        st.table(pd.DataFrame(prob_rows))
                        summary_rows = [
                            {
                                "Metric": "Average total cards",
                                "Value": f"{ref_stats['avg_total_cards']:.2f}" if ref_stats['avg_total_cards'] is not None else "-",
                            },
                            {
                                "Metric": "Average home cards",
                                "Value": f"{ref_stats['avg_home_cards']:.2f}" if ref_stats['avg_home_cards'] is not None else "-",
                            },
                            {
                                "Metric": "Average away cards",
                                "Value": f"{ref_stats['avg_away_cards']:.2f}" if ref_stats['avg_away_cards'] is not None else "-",
                            },
                            {
                                "Metric": f"Average cards to {home_team}",
                                "Value": f"{ref_stats['avg_cards_to_home_team']:.2f}" if ref_stats['avg_cards_to_home_team'] is not None else "-",
                            },
                            {
                                "Metric": f"Average cards to {away_team}",
                                "Value": f"{ref_stats['avg_cards_to_away_team']:.2f}" if ref_stats['avg_cards_to_away_team'] is not None else "-",
                            },
                        ]
